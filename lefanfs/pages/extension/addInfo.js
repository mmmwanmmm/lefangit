...
Page({
  ...
  onLoad: function (options) {
    ...
    var addInfoData = wx.getStorageSync('addInfoData');
    if (addInfoData != null) {
      var data = JSON.parse(addInfoData);
      this.setData({
        optionsData:data,
        name: data.realName,
        phone: data.phone,
  ...
  onUnload: function () {
    // 页面关闭
    wx.setStorageSync('addInfoData', null);
...
var formId = e.detail.formId;
    ...
    var url = "";
    var id =null;
    if(this.data.optionsData!=null){
      url="update-user-promoted";
      id = JSON.parse(this.data.optionsData).id;
    }else{
      url="insert-user-promoted";
    }
    var userToken = wx.getStorageSync('loginUserInfo').userToken;
    var api_sign = md5.hexMD5('api_code='+url+'&api_key=ca89e65c77be0d3f0d732cc3134edaf4&city=' + data.cityCode + '&district=' + data.countyCode + '&formId='+formId+'&id='+id+'&occupation=' + data.zhiye + '&province=' + data.provinceCode + '&phone=' + data.phone + '&realName=' + data.name + '&user_token=' + userToken + '&wechatId='+data.wechatId+'&32f48148e40fcf5586c269f65e6045b5');
    var data = JSON.stringify({ "api_code": url, "api_key": "ca89e65c77be0d3f0d732cc3134edaf4", "user_token": userToken, "city": data.cityCode, "district": data.countyCode,"id":id, "occupation": data.zhiye, "province": data.provinceCode, "phone": data.phone, "realName": data.name,"wechatId":data.wechatId,"formId":formId, "api_sign": api_sign });
    wx.request({
      url: wx.getStorageSync('apiUrl') + 'insert-user-promoted/' + api_sign,
      data: {
        "data": data
      },
      method: 'POST', // OPTIONS, GET, HEAD, POST, PUT, DELETE, TRACE, CONNECTW
