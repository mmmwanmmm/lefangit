        ...
        <table class="table table-hover">
            ...
            <c:forEach items="${loanApplicationList}" var="item">
                <tr>
                    ...
                    <th><a href="javascript:loanDelete('${item.id}');">删除</a>&nbsp;&nbsp;&nbsp;
                        <c:if test="${item.state == 1}">
                        <a href="javascript:loanUpdate('${item.id}',2);">受理</a>
                        <a href="javascript:loanUpdate('${item.id}',3);">驳回</a>
                        </c:if>
                        <c:if test="${item.state == 2}">
                            <a href="javascript:loanUpdate('${item.id}',4);">申请</a>
                        </c:if>
                        <c:if test="${item.state == 4}">
                            <a href="javascript:loanUpdate('${item.id}',5);">贷款完成</a>
                        </c:if>
                    </th>
                </tr>
            </c:forEach>
